...
```bash
cd apps/api
./setup.sh   # one-time: builds .venv from binary wheels (no compiler/cmake needed)
./run.sh     # starts PostgreSQL if needed, then the API on 0.0.0.0:8000
```

`run.sh` binds to all interfaces so the Flutter app on a phone or emulator can
reach it, and prints the exact localhost / LAN / emulator URLs to use. The API is
available at `http://localhost:8000`; check `http://localhost:8000/health` or open
`http://localhost:8000/docs` for the interactive API contract.

> The dependency stack is installed entirely from prebuilt wheels (including
> `dlib-bin` for dlib and an opencv build compatible with NumPy 2 / CPython 3.14),
> so no C++ toolchain is required. See `apps/api/requirements.txt` for details.
...
The Flutter client (in the repository root of this project) talks to the same
API. Point it at your backend in any of these ways:

- **Android emulator:** open the app's **Server settings** and use
  `http://10.0.2.2:8000`.
- **Physical phone (same Wi-Fi):** use your machine's LAN address, e.g.
  `http://<your-ip>:8000` (the address `run.sh` prints).
- **Build-time default:** `flutter run --dart-define=API_BASE_URL=http://<host>:8000/api/v1`.

The `/api/v1` suffix is added automatically if you omit it, cleartext HTTP to a
LAN address is already allowed by the Android manifest, and teacher sign-up uses
`TEACHER_INVITE_CODE` (default `SMART-TEACHER-DEMO`).
...
2. Start the browser camera and complete the guided scan that captures five reference photos (front, each side, up, and down). Individual captures can be retaken or removed before submitting.
